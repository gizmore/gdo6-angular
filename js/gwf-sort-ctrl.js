"use strict";
angular.module('gdo6').
controller('GDOSortCtrl', function($scope, GDORequestSrvc) {
	
	$scope.initJSON = function(config) {
		console.log('GDOSortCtrl.init()', config);
		$scope.config = config;
		setTimeout(function(){
	...
	$scope.onDrop = function(a, b) {
		console.log('GDOSortCtrl.onDrop()', a, b);
	...
	$scope.setupDragDrop = function($elements) {
		console.log('GDOSortCtrl.init()', $elements);
		setTimeout(function(){
			$elements.draggable({
				helper: 'clone',
			});
