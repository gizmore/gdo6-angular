"use strict";
angular.module('gdo6').
controller('GDOTreeCtrl', function($scope) {
	$scope.init = function(options, tree) {
		console.log('GDOTreeCtrl.init()', options, tree);
	...
	$scope.onToggled = function($event, id) {
		...
		root.sel = !root.sel;
		console.log('GDOTreeCtrl.onToggled()', root.label, root.sel);
...
//		console.log('GDOTreeCtrl.toggleChilds()', node, selected);
		for (var i in node.children) {
...
//		$scope.$apply();
	...
	$scope.getSumToggle = function(id) {
		...
		console.log('GDOTreeCtrl.getSumToggle()', node.label, sum);
		return sum;
	};
...
//		console.log('TreeCtrl.isIndeterminate()', $scope.all[id].label, i);
		return i;
	};
